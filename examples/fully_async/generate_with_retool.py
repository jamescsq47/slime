...
WDB_LOG_INTERVAL_SEC = 5.0
request_counter = {"active": 0, "total": 0, "last_log_time": 0.0}
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    mismatch_count = 0  # Track token/logp length mismatches for debugging
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        try:
            ...
            should_log = False
            with counter_lock:
                if now - request_counter["last_log_time"] >= WDB_LOG_INTERVAL_SEC:
                    request_counter["last_log_time"] = now
                    should_log = True
                    active_now = request_counter["active"]
                    total_now = request_counter["total"]

            if wandb.run is not None and should_log:
                wandb.log(
                    {
                        "concurrency/sglang_active_requests": active_now,
                        "concurrency/sglang_total_requests": total_now,
                    }
                )
        except ImportError:
            pass
        ...
        if output["meta_info"]["finish_reason"]["type"] == "abort":
            ...
            sample.mismatch = mismatch_count
            return sample
        ...
        # print(f"Turn {turn}: Added {len(cur_response_token_ids)} tokens, {len(cur_log_probs) if 'cur_log_probs' in dir() else 0} logprobs")
        # print(f"  Total so far: {len(response_token_ids)} tokens, {len(sample.rollout_log_probs) if sample.rollout_log_probs else 0} logprobs")
        ...
        if sample.rollout_log_probs is not None:
            ...
            # print(f"Turn {turn}: Added {len(obs_tokens_ids)} observation tokens with dummy logprobs")
            # print(f"  Total after obs: {len(response_token_ids)} tokens, {len(sample.rollout_log_probs)} logprobs")
...
async def reward_func(args, sample, **kwargs):
    ...
    if result["score"] < 0:
        tool_call_reward = num_turns * 0.1
        # tool_call_reward = num_turns * 0.05
        result["score"] = min(-0.6, result["score"] + tool_call_reward)
    # elif result["score"] > 0:
    #     tool_call_reward = num_turns * 0.05
    #     result["score"] = max(0.7, result["score"] - tool_call_reward)

    if result["pred"] is None:
        result["pred"] = ""

    result["tool_call_count"] = num_turns
    print(f"Reward result: score={result['score']}, num_turns={num_turns}")
    return result
