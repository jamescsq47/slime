...
# Colocate uses SGLang TorchMemorySaver, which requires the native allocator.
if [[ "${PYTORCH_CUDA_ALLOC_CONF:-${PYTORCH_ALLOC_CONF:-}}" == *"expandable_segments:True"* ]]; then
    echo "colocate mode requires the native PyTorch CUDA allocator; unset expandable_segments:True" >&2
    exit 1
fi
export PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF:-${PYTORCH_ALLOC_CONF:-backend:native}}
...
echo "=== Running mixed BrowseComp + Retool colocate + partial rollout training ==="
...
SAVE_PATH=${SAVE_PATH:-/workspace/Qwen3-8B-mixed-mask51200-collocate-partial/}
...
# Optional training-side composition control. In strict_16_16 mode, rollout
# dispatch remains mixed, but only 16 completed Math groups and 16 completed
# QA groups enter each training batch. Extra completed groups wait in the
# existing rollout buffer and are reused without regeneration.
TRAIN_BATCH_BALANCE_MODE=${TRAIN_BATCH_BALANCE_MODE:-none}
TRAIN_BATCH_BALANCE_ARGS=()
case "${TRAIN_BATCH_BALANCE_MODE}" in
   none)
      MATH_RATIO=${MATH_RATIO:-1}
      ;;
   strict_16_16)
      if [ "${ROLLOUT_BATCH_SIZE}" -ne 32 ]; then
         echo "strict_16_16 requires ROLLOUT_BATCH_SIZE=32" >&2
         exit 1
      fi
      MATH_RATIO=${MATH_RATIO:-0.5}
      if ! awk -v ratio="${MATH_RATIO}" 'BEGIN { exit !(ratio > 0 && ratio < 1) }'; then
         echo "strict_16_16 requires 0 < MATH_RATIO < 1 so both tasks can finish" >&2
         exit 1
      fi
      TRAIN_BATCH_BALANCE_ARGS+=(
         --train-batch-math-groups 16
         --train-batch-qa-groups 16
      )
      echo "Training batch balance: strict 16 Math groups + 16 QA groups (dispatch math ratio ${MATH_RATIO})"
      ;;
   *)
      echo "Unknown TRAIN_BATCH_BALANCE_MODE=${TRAIN_BATCH_BALANCE_MODE}; use none or strict_16_16" >&2
      exit 1
      ;;
esac
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-8B-browsecomp-retool-collocate-partial-1.0-51200-51200
)
# W&B credentials are forwarded through the Ray runtime environment below,
# not included in parsed CLI arguments recorded by the run config.
...
ROLLOUT_ARGS=(
   ...
   # Keep eight spare groups in flight so the fastest 32 form the batch.
...
# Keep the rollout window full: each completed group is replaced until the
# training batch reaches ROLLOUT_BATCH_SIZE. Set to 0 to restore one-shot
# over-sampling.
if [ "${ROLLOUT_REPLENISH_COMPLETED_GROUPS:-1}" = "1" ]; then
   ROLLOUT_ARGS+=(--rollout-replenish-completed-groups)
fi
