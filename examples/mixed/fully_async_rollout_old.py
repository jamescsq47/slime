...
async def generate_rollout_async(args, rollout_id: int, data_buffer) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        for group_id in available_ids:
            ...
            # if any_aborted:
            #     # True partial-rollout semantics: only recycle unfinished samples
            #     # when args.partial_rollout is enabled. Otherwise, drop them.
            #     if getattr(args, "partial_rollout", False):
            #         # Mark the rollout where this sample first started, if not already set.
            #         for sample in group:
            #             try:
            #                 if getattr(sample, "response", None):
            #                     if getattr(sample, "metadata", None) is None:
            #                         sample.metadata = {}
            #                     if isinstance(sample.metadata, dict) and "start_rollout_id" not in sample.metadata:
            #                         sample.metadata["start_rollout_id"] = rollout_id
            #             except Exception:
            #                 # Best-effort metadata annotation; don't break rollout on error.
            #                 pass
            ...
            #             data_buffer.add_samples([group])
            #             print(
            #                 f"[partial_rollout] Returned aborted group {group_id} to data buffer (size={len(group)})",
            #                 flush=True,
            #             )
            #         except Exception as e:
            #             print(f"Failed to return aborted group {group_id} to buffer: {e}", flush=True)
            #     else:
            #         # In non-partial mode, we simply drop aborted groups to keep
            #         # semantics aligned with normal rollout (no partial reuse).
            #         print(
            #             f"[partial_rollout disabled] Dropping aborted group {group_id} (size={len(group)})",
            #             flush=True,
            #         )
            #     # don't count as processed for training
            #     continue
    ...
    try:
        ...
        if task_types:
            ...
            math_reward_avg = sum(s.reward["score"] for s in math_samples) / total_math if total_math > 0 else 0
            qa_reward_avg = sum(s.reward["score"] for s in qa_samples) / total_qa if total_qa > 0 else 0
