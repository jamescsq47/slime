...
class CustomDataSource(DataSource):
    ...
    def _mix_samples(self, math_samples, qa_samples):
        ...
        尾部退化成只使用另一类数据，导致实际比例偏离 math_ratio。
        """
        if not math_samples:
            return qa_samples
        if not qa_samples:
        ...
        logger.info(
            f"Mixed normal data with recycling: total={len(mixed)}, "
            f"math={target_math}, qa={target_qa}, math_ratio={target_math / len(mixed):.4f}"
        )
