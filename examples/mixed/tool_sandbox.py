...
TOOL_CONFIGS = {
    ...
    "python_cpu_limit": 1,
    # Tool output settings. This limits stdout/stderr inserted back into the model context.
    "max_tool_output_chars": 8192,
    # Memory management settings
    "max_memory_usage": 12288,  # 12GB total (75% of 16GB)
    "cleanup_threshold": 6144,  # 6GB
    "aggressive_cleanup_threshold": 4096, #3072,  # 3GB
...
def truncate_tool_output(text: str) -> str:
    """Bound tool output before it is appended to the rollout response."""
    max_chars = TOOL_CONFIGS.get("max_tool_output_chars", 8192)
    ...
    return (
        text[:head_chars]
        + f"\n...[truncated {omitted} chars from tool output]...\n"
        + text[-tail_chars:]
    )
...
class PythonSandbox:
    ...
    def _check_code_safety(self, code: str) -> tuple[bool, str]:
        ...
        # Check imported top-level modules. Keep this line-oriented so
        # `from sympy import symbols` validates `sympy`, not `symbols`.
        import_pattern = r"^\s*import\s+([^#\n]+)"
        from_pattern = r"^\s*from\s+([a-zA-Z_][\w.]*)\s+import\s+"

        imports = []
        for match in re.finditer(import_pattern, code, re.MULTILINE):
            for imported in match.group(1).split(","):
                module_name = imported.strip().split()[0].split(".")[0]
                if module_name:
                    imports.append(module_name)

        froms = [match.group(1).split(".")[0] for match in re.finditer(from_pattern, code, re.MULTILINE)]
