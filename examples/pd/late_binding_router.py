...
class LateBindingMiniLoadBalancer(MiniLoadBalancer):
    ...
    def __init__(self, router_args):
        ...
        self._p_ready_submitted_sequences: set[int] = set()
    ...
    def _publish_parent_arrival(self, request: dict[str, Any]) -> None:
        ...
        if metadata is None or metadata.parent is None:
            return
        try:
            payload = store.publish_arrival(metadata.parent)
        except OSError:
    ...
    def _oldest_p_ready_sequence(self) -> Optional[int]:
        sequences: list[int] = []
        for path in self.p_ready_dir.glob("*.ready"):
            try:
                ...
                sequence = payload.get("ready_sequence")
                sequence = (
                    int(sequence) if sequence is not None else path.stat().st_mtime_ns
                )
                if sequence not in self._p_ready_submitted_sequences:
                    sequences.append(sequence)
            except (OSError, orjson.JSONDecodeError, TypeError, ValueError):
                continue
        return min(sequences) if sequences else None

    async def _acquire_p_ready_fifo(self, sequence: int) -> None:
        """Acquire the dispatch lock only for the oldest published P result."""

        while True:
            await self._p_ready_fifo_lock.acquire()
            oldest = self._oldest_p_ready_sequence()
            if oldest is None or sequence <= oldest:
                self._p_ready_submitted_sequences.add(sequence)
                return
            self._p_ready_fifo_lock.release()
            await asyncio.sleep(self.ready_poll_interval)
            ...
            while True:
                async with self._selection_lock:
                    ...
                    for load in loads:
                        ...
                        feasible = free_after_pending >= admission_tokens
                        projected_kv = (
                            load.used_tokens
                            + reserved_admission
                            + admission_tokens
    ...
    async def _release_reservation_when_admitted(
        self, reservation: DecodeReservation, ready_sequence: Optional[int] = None
    ) -> None:
        ...
        try:
            ...
            if ready_sequence is not None:
                self._p_ready_submitted_sequences.discard(ready_sequence)
            async with self._selection_lock:
                if admitted:
    ...
    async def _late_dispatch(
        ...
        admission_wait = await self._prefill_admission.acquire(parent_turn=parent_turn)
        if admission_wait >= 1.0:
            logger.info(
                "PD_P_ADMISSION rooms=%s parent_turn=%s wait_s=%.3f active=%d limit=%d",
                rooms,
                parent_turn,
                admission_wait,
        ...
        try:
            ...
            await self._wait_until_prefill_scheduled(rooms, prefill_task)
            prompt_tokens = await self._wait_until_prefill_ready(rooms, prefill_task)
            ...
            await self._acquire_p_ready_fifo(ready_sequence)
            try:
