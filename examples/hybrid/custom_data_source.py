...
and the search-r1 data loading for QA data, with the only addition being
...
class CustomDataSource(DataSource):
    ...
    def __init__(self, args):
        ...
        if args.rollout_global_dataset:
            ...
            elif self.batch_alternation or self.phase_aware_alternation:
                self._load_data_for_batch_alternation(args, tokenizer, processor)
            else:
                self._load_unified_data(args, tokenizer, processor)
            
            if self.args.rollout_shuffle and not self.batch_alternation and not self.phase_aware_alternation and not self.dynamic_alternation:
    ...
    def _load_qa_data(self, args, tokenizer, processor, qa_path):
        """加载QA数据，使用标准Dataset类，行为与search-r1完全一致"""
        if not qa_path:
            return []
        ...
        samples = dataset.origin_samples
        for s in samples:
            s.metadata = {**(s.metadata or {}), "task_type": "qa", "tools_available": ["search"]}
        logger.info(f"Loaded {len(samples)} QA samples from {qa_path}")
        return samples
    ...
    def _load_unified_data(self, args, tokenizer, processor):
        ...
        if math_ratio == 1.0:
            ...
            math_samples = self._load_math_data(args, tokenizer, processor, math_path)
            qa_samples = []
        elif math_ratio == 0.0:
            if not qa_path:
            ...
            math_samples = []
            qa_samples = self._load_qa_data(args, tokenizer, processor, qa_path)
        else:
        ...
        if not math_samples and not qa_samples:
            raise ValueError("No data loaded! Both math and QA datasets are empty.")

        logger.info(f"Loaded {len(math_samples)} math samples, {len(qa_samples)} QA samples")

        self.origin_samples = self._mix_samples(math_samples, qa_samples)
        self.samples = self.origin_samples
        logger.info(
            f"Loaded {len(self.origin_samples)} total samples "
            f"(math: {len(math_samples)}, qa: {len(qa_samples)})"
        )
    ...
    def _load_data_for_batch_alternation(self, args, tokenizer, processor):
        ...
        if self.phase_aware_alternation:
            logger.info(
                ...
                f"post_update_task={self.phase_aware_post_update_task}, "
    ...
    def _mix_samples(self, math_samples, qa_samples):
        """按比例混合math和qa的Sample列表"""
        if not math_samples:
            return qa_samples
        if not qa_samples:
            return math_samples
        ...
        if self.args.rollout_shuffle:
            ...
            math_samples = sorted(math_samples, key=lambda x: rng.random())
            qa_samples = sorted(qa_samples, key=lambda x: rng.random())
        ...
        while math_ptr < math_len and qa_ptr < qa_len:
            if rng_mix.random() < self.math_ratio:
                mixed.append(math_samples[math_ptr])
                math_ptr += 1
            else:
                mixed.append(qa_samples[qa_ptr])
                qa_ptr += 1

        mixed.extend(math_samples[math_ptr:])
        mixed.extend(qa_samples[qa_ptr:])
    ...
    def get_samples(self, num_samples: int) -> list[list[Sample]]:
        ...
        elif self.phase_aware_alternation:
            samples += self._get_samples_phase_aware(num_samples)
    ...
    def _phase_aware_quota_for_task(self, task_type: str) -> int:
        if task_type == "math":
            return max(0, int(self.math_batches_per_cycle))
        return max(0, int(self.qa_batches_per_cycle))

    def _reset_phase_aware_cycle_if_needed(self):
        math_quota = self._phase_aware_quota_for_task("math")
        qa_quota = self._phase_aware_quota_for_task("qa")
        if (
            self.phase_aware_cycle_counts.get("math", 0) >= math_quota
            and self.phase_aware_cycle_counts.get("qa", 0) >= qa_quota
        ):
            self.phase_aware_cycle_counts = {"math": 0, "qa": 0}

    def _choose_phase_aware_task(self) -> str:
        """Choose task by trainer phase while preserving per-cycle quotas.

        During training, prefer the fast task (default QA). After the policy update,
        prefer the slow task (default math). If the preferred task quota is exhausted,
        fall back to the other task; once both quotas are exhausted, start a new cycle.
        """
        if not hasattr(self, "phase_aware_cycle_counts"):
            self.phase_aware_cycle_counts = {"math": 0, "qa": 0}

        self._reset_phase_aware_cycle_if_needed()
        phase = getattr(self.args, "current_policy_phase", "post_update")
        preferred = self.phase_aware_train_task if phase == "train" else self.phase_aware_post_update_task
        fallback = "qa" if preferred == "math" else "math"

        for task_type in (preferred, fallback):
            quota = self._phase_aware_quota_for_task(task_type)
            if quota > 0 and self.phase_aware_cycle_counts.get(task_type, 0) < quota:
                self.phase_aware_cycle_counts[task_type] = self.phase_aware_cycle_counts.get(task_type, 0) + 1
                return task_type

        # Degenerate zero-quota config: fall back to math_ratio sampling.
        return "math" if random.random() < self.math_ratio else "qa"

    def _get_one_prompt_for_task(self, task_type: str):
        if task_type == "math":
            source_samples = self.math_samples
            offset = self.math_offset
        else:
        ...
        if task_type == "math":
            ...
            self.qa_offset = new_offset
        return prompt_sample, task_type

    def _build_group_from_prompt(self, prompt_sample, current_version: int):
        group = []
        for _ in range(self.args.n_samples_per_prompt):
            sample = copy.deepcopy(prompt_sample)
            sample.group_index = self.sample_group_index
    ...
    def _get_samples_phase_aware(self, num_samples: int) -> list[list[Sample]]:
        """Phase-aware alternation.

        The dispatch stream keeps a fixed quota per cycle (for example 16 QA +
        16 math). While the trainer is busy (`current_policy_phase=train`), it
        consumes the train-task quota first (default QA). After weights are
        updated (`post_update`), it consumes the post-update task quota first
        (default math). This keeps the global ratio controlled while placing
        slow math prompts closer to the newest policy version.
        """
        current_version = getattr(self.args, "current_policy_version", 0)
        phase = getattr(self.args, "current_policy_phase", "post_update")
        samples = []
        ...
        for _ in range(num_samples):
            task_type = self._choose_phase_aware_task()
            prompt_sample, chosen_task = self._get_one_prompt_for_task(task_type)
            if prompt_sample is None:
                fallback = "qa" if task_type == "math" else "math"
                prompt_sample, chosen_task = self._get_one_prompt_for_task(fallback)
            if prompt_sample is None:
                continue
            samples.append(self._build_group_from_prompt(prompt_sample, current_version))
            dispatched[chosen_task] = dispatched.get(chosen_task, 0) + 1

        logger.info(
            f"[v{current_version}] phase_aware_alternation: phase={phase}, "
            f"cycle_counts={self.phase_aware_cycle_counts}, "
            f"dispatched_math={dispatched.get('math', 0)}, dispatched_qa={dispatched.get('qa', 0)}, "
            f"total={len(samples)}"
        )
        return samples
    ...
    def save(self, rollout_id):
        ...
        if self.batch_alternation or self.phase_aware_alternation:
            state_dict.update({
                ...
                "phase_aware_cycle_counts": getattr(self, "phase_aware_cycle_counts", {"math": 0, "qa": 0}),
            })
    ...
    def load(self, rollout_id=None):
        ...
        if (self.batch_alternation or self.phase_aware_alternation) and state_dict.get("batch_alternation", False):
            ...
            self.phase_aware_cycle_counts = state_dict.get("phase_aware_cycle_counts", getattr(self, "phase_aware_cycle_counts", {"math": 0, "qa": 0}))
        ...
        if self.args.rollout_global_dataset and self.args.rollout_shuffle and not self.batch_alternation and not self.phase_aware_alternation:
            self.shuffle(self.epoch_id)
    
    def shuffle(self, new_epoch_id):
        """为新的epoch打乱样本（仅用于普通模式）"""
        if self.batch_alternation or self.phase_aware_alternation:
            logger.warning("shuffle() called in alternation mode, but should use _reshuffle_for_new_epoch()")
            return
