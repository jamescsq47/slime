...
class RolloutManager:
    ...
    def _apply_train_time_offpolicy_mask(self, sample: Sample) -> bool:
        ...
        # Partial generators record the response prefix that predates the
        # latest resume. Mask precisely that stale prefix at training time;
        # the suffix was generated by the current policy and remains trainable.
        if metadata.get("partial_rollout"):
            prefix_length = metadata.get("partial_rollout_prefix_length")
            if prefix_length is not None:
                prefix_length = min(max(int(prefix_length), 0), sample.response_length)
                sample.loss_mask[:prefix_length] = [0] * prefix_length
        else:
