...
from examples.retool.tool_sandbox import SEMAPHORE, TOOL_CONFIGS, tool_registry
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        # print(f"Turn {turn}: total_length={total_length}, prompt_length={len(prompt_tokens_ids)}, response_length={len(response_token_ids)}")
        ...
        if "output_token_logprobs" in output["meta_info"]:
            ...
            # print(f"Turn {turn}: Received log probs for {len(cur_log_probs)} tokens. Total log probs length: {len(sample.rollout_log_probs)}")
        ...
        # print(f"Turn {turn}: Received response of length {len(cur_response_token_ids)} tokens. Total response length: {len(response_token_ids)} tokens.")
        ...
        # print(f"Turn {turn}: Received observation of length {len(obs_tokens_ids)} tokens. Total response length: {len(response_token_ids)} tokens.")
        ...
        if sample.rollout_log_probs is not None:
            ...
            # print(f"Turn {turn}: Added {len(obs_tokens_ids)} dummy log probs for observation tokens. Total log probs length: {len(sample.rollout_log_probs)}")
            ...
            if len(response_token_ids) != len(sample.rollout_log_probs):
                print(f"Sample discarded due to length mismatch")
                print(f"Response token ids: {len(response_token_ids)}")
                print(f"Rollout log probs length: {len(sample.rollout_log_probs)}")
                print(f"Current response: {len(response)}")
                print(f"Current next_obs: {next_obs}")
                print(f"Loss masks length: {len(loss_masks)}")
    ...
    sample.reward = result["score"]
    args.reward_key = False
